import { View, ScrollView, Text, Pressable, TextInput, Alert, StyleSheet, ToastAndroid, Button, ActivityIndicator, Modal, TouchableOpacity, Image } from 'react-native'
import React, { useEffect, useState, useMemo, createContext, useDebugValue } from 'react'
...
export default function Taskscsh({ navigation }) {
  ...
  const [caseused, setCase] = useState(locale === "ar_MA" ? "إستلام" : "Receive");
  ...
  const onPress = async () => {
    if (sumCash.length === 0 && selectedStartDateS.length === 0 && selectedStartDateF.length === 0 && caseuTarg.length === 0 && codm.length <= 0) {
      ...
      ToastAndroid.showWithGravity(locale === "ar_MA" ? 'لايمكن ان يكون المبلغ المسدد اعلى من الاجمالي' : "The paid amount cannot be higher than the total",
        ToastAndroid.CENTER,
        ToastAndroid.SHORT
      )
      setFalse(false)
    } else {
      console.log( dolars.find(item => item.value == codm.value)?.label)
      try {
        setFalse(true)
        var Task = {
          ID: tasksCSHID,
          caseuTarg: caseuTarg,
          SumCash: sumCash,
          codm: dolars.find(item => item.value == codm.value)?.label,
        ...
        const index = tasksCSH.findIndex((task: { ID: any }) => task.ID == tasksCSHID)
        let newTasks: any[] = [];
        if (index > -1) {
          newTasks = [...tasksCSH];
          newTasks[index] = Task;
  ...
  const empyte = () => {
    setDesc('')
    setCase(locale === "ar_MA" ? 'إستلام' : "Receive")
    setPhonNmber("")
    setFalse(false)
    setSumCsh('')
  }
  ...
  const deletImage = () => {
    RNFS.unlink(fileurl)
      .then(() => {
        const index = tasksCSH.findIndex((tasks: { ID: any }) => tasks.ID === tasksCSHID);
        if (index > -1) {
  ...
  return (
    <>
      ...
      {calculatorCover ? <ModulsCalculator visble={calculatorCover} onrequewt={setCalculatorCover} onprssfounction={() => useclurekD()} /> : null}
      <Modal
        visible={liprri}
        transparent
        ...
        animationType='slide'
        hardwareAccelerated>
        <TouchableOpacity onPress={() => setLiprre(false)} style={styles.centered_IMag}>
          <View style={styles.mod1al}>

            <View style={styles.button}>
              <Pressable
                android_ripple={{ color: colors.YALO }}
                onPress={() => { launch(2);; setLiprre(false) }}
                style={styles.im}>
                <FontAwesome5Icon
                  name='images'
                  size={30}
              ...
              <Pressable
                android_ripple={{ color: colors.YALO }}
                onPress={() => { launch(1); setLiprre(false) }}
                style={styles.im}>
                <FontAwesome5Icon
                  name='camera'
                  size={30}
      ...
      <View style={styles.body}>
        <View style={styles.puchcontener}>
          <TouchableOpacity onPress={() => setCase(locale === "ar_MA" ? 'إستلام' : "Receive")} style={[caseused === 'إستلام' || caseused === "Receive" ? { backgroundColor: colors.CURRENT } : null, styles.puchcontener_sub1]}>
            <Text style={[caseused === 'إستلام' || caseused === "Receive" ? { color: colors.WHITE } : { color: colors.CURRENT }, styles.puchcontener_sub1_text]}>{locale === "ar_MA" ? "استلام" : "Receive"}</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setCase(locale === "ar_MA" ? 'دفع' : "To push")} style={[caseused === 'دفع' || caseused === "To push" ? { backgroundColor: colors.CURRENT } : null, styles.puchcontener_sub1]}>
            <Text style={[caseused === 'دفع' || caseused === "To push" ? { color: colors.WHITE } : { color: colors.CURRENT }, styles.puchcontener_sub1_text]}>{locale === "ar_MA" ? "دفع" : "To push"}</Text>
          </TouchableOpacity >
        </View>
        <ScrollView style={{ marginBottom: RFValue(30) }}>
          <View style={styles.mossdd}>
            <TextInput style={styles.inputdecerb} placeholderTextColor={colors.CURRENT} placeholder={locale === "ar_MA" ? caseused === 'دفع' ? "الدائن" : "المدين" : caseused === "To push" ? "Creditor" : " the debtor"} value={caseuTarg} onChangeText={(value) => setTarget(value)} />
            <View style={styles.inputtitelcounterinput}>
              ...
              <TextInput style={styles.inputdecerb} placeholderTextColor={colors.CURRENT} keyboardType='number-pad' placeholder={locale === "ar_MA" ? "اجمالي الدين" : "Total debt"} value={sumCash} onChangeText={(value) => setSumCsh(value)} />
            </View>
          </View>
          ...
          <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-around' }}>
            <View style={styles.dtatastart}>
              <TouchableOpacity onPress={() => {
                ...
                <Text style={styles.textmos}>{locale === "ar_MA" ? "تاريخ البدء" : "Starting Date"}</Text>
            ...
            <View style={styles.dtatastart}>
              <TouchableOpacity onPress={() => {
                ...
                <Text style={styles.textmos}>{locale === 'ar_MA' ? "تاريخ الانتهاء" : "Expiry Date"}</Text>
          ...
          <View style={styles.mossdd}>
            ...
            <View style={styles.inputtitelcounterinput}>
              <Pressable android_ripple={{ color: colors.YALO, borderless: true }} style={{ top: 20, left: 30, zIndex: 1 }} onPress={() => setCalculatorCover(true)}>
                <FontAwesome5Icon name='calculator' size={15} color={colors.GREYD} />
              </Pressable>
              <TextInput style={styles.inputdecerb} keyboardType='number-pad' placeholder={locale === "ar_MA" ? "المبلغ المدفوع" : "Payment"} value={desc} onChangeText={(value) => setDesc(value)} />
            </View>
          </View>
          <View style={styles.mossdd}>
          ...
          <View style={styles.viewnotic}>
            ...
            {False
              ?
              <ActivityIndicator style={{ flex: 2 }} color={colors.RED} size={25} />
              :
              <TouchableOpacity
                onPress={onPress}
                style={styles.set}>
