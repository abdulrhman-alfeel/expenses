...
export default function About({navigation}) {
  ...
  const delet = (id) => {
    ...
    AsyncStorage.setItem('Tasks', JSON.stringify(filterDasec)).then(() => {
...
if(fileExistS){
  ...
  const Shaer = async(items,Title)=>{
    try {
      const result = await  Share.share({
        message:JSON.stringify(items.map(ite=>ite)),
  ...
  return (
    ...
    <View style={styles.senction3}>
          <TouchableOpacity onPress={() => {
           setPagSec('Crueat')}} style={[pagSec == 'Crueat' ? styles.conter : null, styles.contsec]}>
          ...
          <TouchableOpacity onPress={() => setPagSec('Fanshing')} style={[pagSec == 'Fanshing' ? styles.conter : null, styles.contsec]}>
            <Text style={styles.text}>{locale === 'ar_MA'?"المنتهيه":"Ended"}</Text>
          </TouchableOpacity>
         
        </View>
      <FlatList
        data={tasks.filter(item => (pagSec == 'Fanshing'? item.Done ===  true:item.Done === false))}
        renderItem={({ item, index }) => (
          <View style={styles.tasksbox} >
            ...
            <TouchableOpacity onPress={() => {
             ...
             navigation.navigate("Tasks")
              ...
              <View style={styles.item_body}>
                <Text style={styles.texttask} numberOfLines={1}>{item.Title}</Text>
                <Text style={styles.textdesc} numberOfLines={1}>{item.dataimag[0].docements}</Text>
              </View>
            ...
            <Pressable
              android_ripple={{color:colors.WHITE}}
                style={{backgroundColor:colors.GREYD,alignItems:'center',justifyContent:'center',left:RFValue(10),width:RFValue(40),borderRadius:RFValue(10), height:RFValue(20)}}
                onPress={() => {
                  // let opbject=[];
                  // item.dataimag.forEach(pic=>{
                  //   opbject.push(pic.docements)
                  // })
                  // Shaer(opbject,item.Title)
